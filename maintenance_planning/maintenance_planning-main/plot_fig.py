...
font = {"fontname": 'Times New Roman', "fontsize": 10}
font1 = {"fontname": 'Times New Roman', "fontsize": 10}
costs = np.load('./result/training results/step50000/costs.npy')
loss = np.load('./result/training results/step50000/DQNloss.npy')
A = np.reshape(costs[1:-1], [500,100])
B = np.reshape(loss[:-1], [499,100])
fig, ax1 = plt.subplots()
ax1.plot(np.arange(1,501), -np.log10(-np.mean(A, 1)),linewidth=1, color='g')
plt.yticks([-np.log10(30), -np.log10(20), -np.log10(10), -np.log10(5), -np.log10(4), -np.log10(3), -np.log10(2),-np.log10(1)],
           ['-30','-20','-10','-5','-4','-3','-2','-1'])
ax2 = ax1.twinx()
ax2.plot(np.arange(1,500), np.log10(np.mean(B, 1)),linewidth=1, color='b')
plt.yticks([np.log10(0.0001), np.log10(0.001), np.log10(0.01), np.log10(0.1), np.log10(1)],
           ['0.0001','0.001','0.01','0.1','1'])
ax1.set_xlabel('Number of episodes', **font)
ax1.set_ylabel('Total costs', **font, color = 'g')
ax2.set_ylabel('DQN loss', **font, color = 'b')
plt.xlim([-50, 550])
plt.xticks(np.arange(0,501, 100), ['0', '10000', '20000', '30000', '40000', '50000'], **font1)
plt.savefig('./figures/cost&loss1.pdf')
plt.savefig('./figures/cost&loss1.eps')
plt.savefig('./figures/cost&loss1.svg')
